...
2. ПЕРСПЕКТИВНЫЕ ОТРАСЛИ
   ...
   - Объясни почему именно эти отрасли
   - Дай конкретные советы по входу в эти сферы

3. ГОСУДАРСТВЕННАЯ ПОДДЕРЖКА
   - Перечисли все доступные программы поддержки
   - Укажи условия получения и размеры поддержки
   - Дай пошаговые инструкции по подаче заявок

4. ВЫЗОВЫ И РИСКИ
   - Опиши основные проблемы МСП
   - Предложи способы их решения
   - Дай советы по минимизации рисков

5. ПРАКТИЧЕСКИЕ РЕКОМЕНДАЦИИ
   - Конкретные шаги для развития бизнеса
   - Советы по оптимизации процессов
   - Рекомендации по привлечению клиентов

Используй структурированную подачу информации, приводи конкретные примеры и цифры!
"""
            if query:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "Ты - эксперт по малому и среднему бизнесу в России. Давай подробные и структурированные ответы с практическими рекомендациями."},
